...
async fn main() -> io::Result<()> {
    ...
    let privkey = hello_world::load_private_key("static/pki/client.key")
        .map_err(|err| error(format!("load client private key: {err}")))?;

    let certs = hello_world::load_certs("static/pki/client.crt")
        .map_err(|err| error(format!("load client certs: {err}")))?;
